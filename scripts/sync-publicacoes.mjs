...
const CPF = process.env.FAZ_CPF;
const SENHA = process.env.FAZ_SENHA;

if (!CPF || !SENHA) {
  console.error('Defina FAZ_CPF e FAZ_SENHA nas variáveis de ambiente.');
  process.exit(1);
}
...
async function run() {
  ...
  try {
    ...
    const loginFailed = await page.url().includes('login').catch(() => false);
    if (loginFailed) {
      throw new Error('Falha na autenticação: verifique credenciais FAZ_CPF e FAZ_SENHA');
    }
